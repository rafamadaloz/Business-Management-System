...
from SGEO.apps.fiscal.models import NotaFiscalSaida, NotaFiscalEntrada, ConfiguracaoNotaFiscal, AutXML, \
...
from SGEO.apps.cadastro.models import COD_UF, PessoaJuridica, PessoaFisica, Fornecedor, Cliente, Empresa, Transportadora, Endereco, Telefone, Produto, Unidade
from SGEO.apps.compras.models import PedidoCompra, ItensCompra
from SGEO.apps.vendas.models import PedidoVenda, ItensVenda

from SGEO.apps.vendas.models import Pagamento as PagamentoVenda
...
class ProcessadorNotaFiscal(object):
    ...
    def emitir_nota_myse(self, nota_obj):

        self.verificar_configuracao_nfe(nota_obj)
        if not self.info_certificado:
            return self.salvar_mensagem(message='Emissão de NF-e não configurada.', erro=True)

        url = 'https://indefinido'

        token = 'token'

        dados_nfe = {
            'numero': nota_obj.n_nf_saida,
            'serie': nota_obj.n_nf_saida,
            'cfop': nota_obj.cfop,
            'cliente': nota_obj.emit_saida.cpf_cnpj_apenas_digitos,
        }
    ...
    def emitir_nota(self, nota_obj):
        ...
        # token = "16rCZo3bETtChdeofmMUVqRsKNb2OT0w" #Produção

        token = "TupJqHrhbAR2coAPUHxwqHuGgRD2TvhN" # Homologação

        ref = {"ref": nota_obj.n_nf_saida}

        dados_nfe = {
            "natureza_operacao": str(nota_obj.natop),
            "data_emissao": nota_obj.dhemi.isoformat(),
            "data_entrada_saida": nota_obj.dhsaient.isoformat(),
            "tipo_documento": nota_obj.tpnf,
            "finalidade_emissao": nota_obj.fin_nfe,
            "cnpj_emitente": nota_obj.emit_saida.cpf_cnpj_apenas_digitos,
            "incricao_estadual_emitente": nota_obj.emit_saida.pessoa_jur_info.inscricao_estadual,
            "nome_emitente": nota_obj.emit_saida.nome_razao_social,
            "nome_fantasia_emitente": nota_obj.emit_saida.pessoa_jur_info.nome_fantasia,
            "logradouro_emitente": nota_obj.emit_saida.endereco_padrao.logradouro,
            "numero_emitente": nota_obj.emit_saida.endereco_padrao.numero,
            "bairro_emitente": nota_obj.emit_saida.endereco_padrao.bairro,
            "municipio_emitente": nota_obj.emit_saida.endereco_padrao.municipio,
            "uf_emitente": nota_obj.emit_saida.endereco_padrao.uf,
            "cep_emitente": nota_obj.emit_saida.endereco_padrao.cep,
            "nome_destinatario": nota_obj.dest_saida.nome_razao_social,
            "logradouro_destinatario": nota_obj.dest_saida.endereco_padrao.logradouro,
            "numero_destinatario": nota_obj.dest_saida.endereco_padrao.numero,
            "bairro_destinatario": nota_obj.dest_saida.endereco_padrao.bairro,
            "municipio_destinatario": nota_obj.dest_saida.endereco_padrao.municipio,
            "uf_destinatario": nota_obj.dest_saida.endereco_padrao.uf,
            "pais_destinatario": nota_obj.dest_saida.endereco_padrao.pais,
            "cep_destinatario": nota_obj.dest_saida.endereco_padrao.cep,
            "valor_frete": float(nota_obj.venda.frete),
            "valor_seguro": float(nota_obj.venda.seguro),
            "valor_total": float(nota_obj.venda.valor_total),
            "valor_produtos": float(nota_obj.venda.get_valor_total_attr('vprod')),
            "modalidade_frete": nota_obj.venda.mod_frete,
        }

        if nota_obj.dest_saida.telefone_padrao:
            dados_nfe["telefone_destinatario"] = nota_obj.dest_saida.telefone_padrao.get_telefone_apenas_digitos(),

        if nota_obj.emit_saida.pessoa_jur_info.sit_fiscal == 'SN':
            dados_nfe['regime_tributario_emitente'] = '1'
        elif nota_obj.emit_saida.pessoa_jur_info.sit_fiscal == 'SE':
            dados_nfe['regime_tributario_emitente'] = '2'
        elif nota_obj.emit_saida.pessoa_jur_info.sit_fiscal in ('LR', 'LP'):
            dados_nfe['regime_tributario_emitente'] = '3'

        if nota_obj.dest_saida.tipo_pessoa == 'PF':
            dados_nfe['cpf_destinatario'] = nota_obj.dest_saida.cpf_cnpj_apenas_digitos
            dados_nfe['inscricao_estadual_destinatario'] = None
        elif nota_obj.dest_saida.tipo_pessoa == 'PJ':
            dados_nfe['cnpj_destinatario'] = nota_obj.dest_saida.cpf_cnpj_apenas_digitos
            dados_nfe['inscricao_estadual_destinatario'] = nota_obj.dest_saida.inscricao_estadual
        ...
        for index, item in enumerate(nota_obj.venda.itens_venda.all(), 1):
            produto = {
                ...
                "cfop": item.produto.get_cfop_padrao(),
                "unidade_comercial": item.produto.get_sigla_unidade(),
                "quantidade_comercial": float(item.quantidade),
                "valor_unitario_comercial": float(item.valor_unit),
                "valor_unitario_tributavel": float(item.valor_unit),
                "unidade_tributavel": item.produto.get_sigla_unidade(),
                "quantidade_tributavel": float(item.quantidade),
                "valor_bruto": float(item.vprod),
                "icms_origem": item.produto.origem,
            }
            if item.produto.ncm:
                produto['codigo_ncm'] = item.produto.ncm[0:8]

            if item.produto.grupo_fiscal:
                if item.produto.grupo_fiscal.regime_trib == '1':
                    if item.produto.grupo_fiscal.icms_sn_padrao:
                        icms_sn_obj = item.produto.grupo_fiscal.icms_sn_padrao.get()
                        produto['icms_situacao_tributaria'] = icms_sn_obj.mod_bcst

                elif item.produto.grupo_fiscal.regime_trib == '0':
                    if item.produto.grupo_fiscal.icms_padrao:
                        icms_obj = item.produto.grupo_fiscal.icms_padrao.get()
                        produto['icms_situacao_tributaria'] = icms_obj.cst.replace('r', '').replace('p', '')

                pis_obj = item.produto.grupo_fiscal.pis_padrao.get()
                if pis_obj.cst:
                    produto["pis_situacao_tributaria"] = pis_obj.cst
                else:
                    produto["pis_situacao_tributaria"] = '99'

                cofins_obj = item.produto.grupo_fiscal.cofins_padrao.get()

                if cofins_obj.cst:
                    produto["cofins_situacao_tributaria"] = cofins_obj.cst
                else:
                    produto["cofins_situacao_tributaria"] = '99'
        ...
        for k, v in dados_nfe.items():
            types.append(v)

        # return self.salvar_mensagem(message=str(dados_nfe), erro=False)

        r = requests.post(url, params=ref, data=json.dumps(dados_nfe), auth=(token, ""))

        if r.status_code == 200:
            nota_obj.status_nfe = u'1'
            nota_obj.chave = 'chave_gerada'
            nota_obj.arquivo_proc = "caminho xml"
            nota_obj.save()
            return self.salvar_mensagem(message=str(r.text), erro=False)
        else:
            e = ErrosValidacaoNotaFiscal(nfe=nota_obj)
            e.tipo = u'0'
            e.descricao = str(r.text)
            e.save()
            return self.salvar_mensagem(message=str(r.text), erro=True)



    def emitir_nota2(self, nota_obj):
