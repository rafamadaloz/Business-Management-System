...
from SGEO.apps.base.custom_views import CustomView, CustomCreateView, CustomListView, CustomUpdateView, CustomTemplateView

from SGEO.apps.fiscal.forms import NotaFiscalSaidaForm, NotaFiscalEntradaForm, AutXMLFormSet, ConfiguracaoNotaFiscalForm, EmissaoNotaFiscalForm, CancelamentoNotaFiscalForm, \
    ConsultarCadastroForm, InutilizarNotasForm, ConsultarNotaForm, BaixarNotaForm, ManifestacaoDestinatarioForm
from SGEO.apps.fiscal.models import NotaFiscalSaida, NotaFiscalEntrada, NotaFiscal, ConfiguracaoNotaFiscal, AutXML, ErrosValidacaoNotaFiscal, RespostaSefazNotaFiscal
...
class AdicionarNotaFiscalView(CustomCreateView, NotaFiscalViewMixin):
    ...
    def post(self, request, form_class, *args, **kwargs):
        ...
        # Contador.conta_nfe.update(Contador.conta_nfe.value + 1)
...
class NotaFiscalSaidaListView(NotaFiscalListView):
    ...
    def view_context(self, context):
        ...
        context['saida'] = True
        if ConfiguracaoNotaFiscal.objects.exists():
            context['conf_nfe'] = ConfiguracaoNotaFiscal.objects.all()[:1].get()
        else:
            context['conf_nfe'] = False
        return context
...
class EmitirNotaView(CustomTemplateView):
    ...
    def emitir_nota(self):
        processador_nota = ProcessadorNotaFiscal()
        processador_nota.emitir_nota_myse(self.object)
