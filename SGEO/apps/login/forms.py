...
from .models import Usuario, ConfiguracoesDoPlano
...
class PerfilUsuarioForm(forms.ModelForm):
    ...
    user_foto = forms.ImageField(widget=forms.FileInput(
        attrs={'class': 'upload', 'id': 'uploadBtn', 'accept': 'image/*'}), label='Foto de perfil', required=False)
    user_capa_foto = forms.ImageField(widget=forms.FileInput(
        attrs={'class': 'upload', 'id': 'uploadBtn2', 'accept': 'image/*'}), label='Foto de capa', required=False)
    ...
    class Meta:
        model = Usuario
        fields = ('first_name', 'last_name', 'username', 'email', 'user_foto', 'user_capa_foto')
...
class ConfiguracoesDoPlanoForm(forms.ModelForm):
    class Meta:
        ...
        widgets = {
            ...
            'notas_adicional': forms.Select(attrs={'class': 'form-control'}),
            'usuarios_adicional': forms.Select(attrs={'class': 'form-control'}),
        }
        labels = {
            'plano': 'Plano escolhido',
            'notas_adicional': 'Notas adicionais',
