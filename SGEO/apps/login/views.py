...
from SGEO.apps.base.views_mixins import SuperUserRequiredMixin

from .forms import UserLoginForm, UserRegistrationForm, PasswordResetForm, SetPasswordForm, PerfilUsuarioForm
from .models import Usuario
from SGEO.configs.settings import DEFAULT_FROM_EMAIL

from SGEO.apps.cadastro.forms import MinhaEmpresaForm
from SGEO.apps.cadastro.models import MinhaEmpresa
from SGEO.apps.login.forms import ConfiguracoesDoPlanoForm
from SGEO.apps.login.models import ConfiguracoesDoPlano, NotaAdicionalPlano, UsuarioAdicionalPlano
...
DEFAULT_PERMISSION_MODELS = ['cliente', 'fornecedor', 'produto',
                             ...
                             'oportunidade', 'etiqueta', 'configuracaoboleto', 'boleto']
...
CUSTOM_PERMISSIONS = ['configurar_nfe', 'emitir_notafiscal', 'cancelar_notafiscal', 'gerar_danfe', 'consultar_cadastro',
                      ...
                      'baixar_notafiscal', 'manifestacao_destinatario', 'faturar_pedidovenda', 'faturar_pedidocompra', 'acesso_fluxodecaixa', 'consultar_estoque', 'acesso_events',
                      'acesso_navegacao', 'acesso_funil', 'acessar_hotmart', 'acessar_eduzz', 'acessar_mp', 'acessar_ps',
                      'acessar_relatorio_clientes', 'acessar_relatorio_entradas', 'acessar_relatorio_saidas',
                      'acessar_relatorio_produtos', 'acessar_relatorio_vendas', 'acessar_relatorio_dre',
                      ]
            ...
            if associated_user:
                try:
                    if associated_user.email:
                        ...
                        subject = u"Redefinir sua senha - SGEO"
                        ...
                        if sended == 1:
                            messages.success(request, u'Um email foi enviado para ' + associated_user.email +
    ...
    def post(self, request):
        ...
        if form.is_valid() and minha_empresa_form.is_valid():
            try:
                ...
                if 'user_capa_foto' in request.FILES:
                    perfil.user_capa_foto = request.FILES['user_capa_foto']
                perfil.save()
...
class ConfiguracoesPlanoView(UpdateView):
    form_class = ConfiguracoesDoPlanoForm
    template_name = 'login/plano/configuracoes.html'
    success_url = reverse_lazy('login:configuracoesplanoview')
    success_message = "Perfil editado com sucesso."
    ...
    def get(self, request, *args, **kwargs):
        super(ConfiguracoesPlanoView, self).get(request, *args, **kwargs)
        form_class = self.get_form_class()
        form = self.get_form(form_class)
    ...
    def post(self, request):
        ...
        try:
            instance = ConfiguracoesDoPlano.objects.get(user=request.user)
            form = self.form_class(
                request.POST, request.FILES, instance=instance)
        except ConfiguracoesDoPlano.DoesNotExist:
            form = self.form_class(request.POST, request.FILES, instance=None)
...
class InfoNotaAdicional(View):

    def post(self, request, *args, **kwargs):
        obj_list = []
        nota_adicional = NotaAdicionalPlano.objects.get(pk=request.POST['notaAdicionalId'])
        obj_list.append(nota_adicional)

        if nota_adicional.quatidade:
            obj_list.append(nota_adicional.quantidade)
        if nota_adicional.email_padrao:
            obj_list.append(nota_adicional.preco)

        data = serializers.serialize('json', obj_list, fields=('quantidade', 'preco',))
